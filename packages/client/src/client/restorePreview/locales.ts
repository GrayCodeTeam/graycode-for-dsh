...
export const graycodeRestorePreviewDictionaries: Record<LocaleId, LocaleDictOf<'graycode.restorePreview'>> = {
  zh: {
    ...
    'error.endpointNotFound': 'DSH 未提供恢复端点（未接入）。当前为演示模式，不会写入任何文件',
    ...
    tokenHint: 'token 绑定存档、工作区与预览基线；缺失、过期或错配会被 DSH 拒绝（GRAY_APPROVAL_REQUIRED / GRAY_CONFLICT）',
    ...
    totalLabel: '影响文件',
    moreLabel: '另有 {n} 项',
    mockModeHint: '演示模式：DSH 端点未接入，界面使用本地模拟数据，不会写入任何文件',
  ...
  en: {
    ...
    'error.endpointNotFound': 'DSH restore endpoints unavailable (not connected). Running in demo mode — nothing will be written',
    ...
    tokenHint: 'The token binds checkpoint, workspace and preview baseline; missing, expired or mismatched tokens are denied by the DSH (GRAY_APPROVAL_REQUIRED / GRAY_CONFLICT)',
    ...
    moreLabel: '{n} more',
    mockModeHint: 'Demo mode: DSH endpoints are not connected; the panel uses local simulated data and writes nothing',
...
export const graycodeRestorePreviewJaPlaceholder: LocaleDict = {
  ...
  'error.endpointNotFound': 'DSH の復元エンドポイントが未接続です。現在はデモモードで動作し、ファイルは書き込まれません',
  ...
  tokenHint: 'トークンはチェックポイント・ワークスペース・プレビュー基準に紐づきます。欠落・期限切れ・不一致は DSH に拒否されます（GRAY_APPROVAL_REQUIRED / GRAY_CONFLICT）',
  ...
  totalLabel: '影響ファイル',
  moreLabel: '他 {n} 件',
  mockModeHint: 'デモモード：DSH のエンドポイントが未接続のため、ローカルの模擬データを使用します。ファイルは書き込まれません',
