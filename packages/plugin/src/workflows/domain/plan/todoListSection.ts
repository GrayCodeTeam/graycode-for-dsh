/**
 * Plan 文档中的 TODO LIST 章节处理工具（纯函数，不依赖 ctx）
 *
 * 从源 gray-code-plugin `backend/tools/plan/todoListSection.ts` 移植；渲染格式保持逐字一致：
 *
 * ```md
 * ## TODO LIST
 *
 ...
 * ```
 *
 * 语义沿用「宽松归一化」（跳过非法条目、status 默认 pending、重复 id 后者覆盖、
 * 按 id 数值排序），与 domain/shared/todoValidation.ts 注释中保留的 plan 差异一致
 * （progress/todo_write 走严格校验，plan 走本地宽松归一化）。
 */

import { escapeRegExp, normalizeSingleLineText } from '../shared/textUtils.ts';
import { isTodoStatus, type TodoStatus } from '../shared/todoValidation.ts';
...
export const PLAN_TODO_SECTION_TITLE = '## TODO LIST';
export const PLAN_TODO_SECTION_START = '<!-- GRAYCODE_TODO_LIST_START -->';
export const PLAN_TODO_SECTION_END = '<!-- GRAYCODE_TODO_LIST_END -->';
...
export function stripPlanTodoListSection(content: string): string {
  const start = content.indexOf(PLAN_TODO_SECTION_START);
  const end = start >= 0 ? content.indexOf(PLAN_TODO_SECTION_END, start + PLAN_TODO_SECTION_START.length) : -1;
  if (start < 0 || end < 0 || end < start) {
    return content;
  }

  const removeFrom = (() => {
    // 仅当标题独占一行（行首）且与 START 标记之间只有空白时才回退删除标题行；
    // 否则一律按标记块定位（从 START 标记开始删），正文中出现的 "## TODO LIST" 文本不受影响。
    const headingIndex = content.lastIndexOf(PLAN_TODO_SECTION_TITLE, start);
    if (headingIndex < 0) return start;
    const atLineStart = headingIndex === 0 || content[headingIndex - 1] === '\n';
    if (!atLineStart) return start;
    const between = content.slice(headingIndex + PLAN_TODO_SECTION_TITLE.length, start);
    if (!/^\s*$/.test(between)) return start;
    return headingIndex;
  })();
  const removeTo = end + PLAN_TODO_SECTION_END.length;
...
function extractTodoSection(content: string): string {
  ...
  const start = normalized.indexOf(PLAN_TODO_SECTION_START);
  const end = start >= 0 ? normalized.indexOf(PLAN_TODO_SECTION_END, start + PLAN_TODO_SECTION_START.length) : -1;
  if (start >= 0 && end > start) {
    return normalized.slice(start + PLAN_TODO_SECTION_START.length, end);
  }
