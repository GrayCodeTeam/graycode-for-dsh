...
import { normalizeLineEndingsToLF, normalizeSingleLineText } from '../shared/textUtils.ts';
import { findDuplicateIds, isTodoStatus, validateTodos } from '../shared/todoValidation.ts';
import {
...
export function getLatestProgressMilestone(
  ...
  return metadata.milestones.length > 0
    ? (metadata.milestones[metadata.milestones.length - 1] ?? null)
    : null;
}
