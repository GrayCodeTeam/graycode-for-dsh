 ...
 */
import type { CheckpointWorkspaceRoot } from './CheckpointWorkspace.ts';

/**
 * 轻量存档摘要（CPF-02/CPF-03）。
 *
 * DSH 移植：源仓库 T16 起 CheckpointSummary / CheckpointSummaryWithSize 单一来源
 * 位于 shared/protocol.ts；DSH 工程无该共享层，按原契约内联本文件（字段零改动）。
 */
...
/**
 * 单文件清单条目（内容寻址布局，V2 §7.6）。
 *
 * 内容寻址下 blob 不可变：易变字段（mtime 等）不再随存档存储，
 * 只保存 path → 内容哈希映射 + 恢复所需的 size/mode。
 */
export interface CheckpointManifestFileEntry {
    /** 内容哈希（sha256，同时也是 blobs/<hash> 的寻址键） */
    hash: string;
    /** 文件字节数（staging 校验用） */
    size: number;
    /** 文件权限位（fs.Stats.mode；恢复时 best-effort chmod） */
    mode: number;
}

/**
 * 独立存档 manifest（内容寻址布局，schema version 3，V2 §7.6）。
 *
 * 存放于 `<dataRoot>/checkpoints/<workspace-id>/manifests/<checkpoint-id>.json`（单文件，
 * 原子写：tmp + rename）。`files` 为完整文件清单（scopedPath → blob 哈希），
 * `changes` 为相对父检查点的差异（增量父链）；恢复沿父链校验 changes 与 files 一致性。
 */
export interface CheckpointManifest {
    /** manifest schema 版本（CHECKPOINT_MANIFEST_VERSION） */
    version: number;
    checkpointId: string;
    /** 工作区根目录集合（CP-01：恢复前校验当前工作区身份） */
    workspaceRoots: CheckpointWorkspaceRoot[];
    workspaceFingerprint: string;
    /** 增量父链：父检查点 ID（缺省 = 全量快照） */
    parentCheckpointId?: string;
    /** 创建时间戳 */
    createdAt: number;
    /** scopedPath -> 文件信息（完整文件集；哈希即 blobs/<hash> 寻址键） */
    files: Record<string, CheckpointManifestFileEntry>;
    /** 相对父检查点的变更（added/modified/deleted；added/modified 携带 hash） */
    changes: CheckpointFileChange[];
    /** 空目录列表（scoped 路径） */
    emptyDirs: string[];
    /** 被排除路径的完整清单（恢复时解释「为什么没有备份」） */
    excluded: CheckpointExcludedEntry[];
    /** 快照时的排除规则快照（EX-10） */
    ignoreSnapshot: CheckpointIgnoreSnapshot;
    /** 排除规则版本（快照时 ignoreSnapshot.version） */
    excludeRuleVersion: number;
    /** 综合内容签名：按 scopedPath 排序的 `path:hash` 摘要 */
    contentHash: string;
    /**
...
export interface CheckpointRecord {
     ...
     */
    partial?: boolean;
    /** 内容寻址布局：manifest 载荷的内容哈希（preview 门闸绑定用） */
    manifestHash?: string;
    /** 排除规则版本（快照时 ignoreSnapshot.version，V2 §7.6 domain 记录字段） */
    excludeRuleVersion?: number;
    /** 本次提交引用的去重 blob 数（domain 记录字段） */
    blobCount?: number;
    /** 存档状态（active = 正常可用；删除即移出 records） */
    status?: 'active';
