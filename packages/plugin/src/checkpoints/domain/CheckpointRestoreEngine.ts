...
export interface RestoreChainEntry {
    checkpointId: string;
    /** 存档目录名（位于 checkpointsDir 下；内容寻址布局下仅作记录兼容，不再承载文件） */
    backupDir: string;
    /** 该存档记录的文件哈希（scoped 或相对路径键；增量节点记录的是完整工作区映射） */
    fileHashes?: Record<string, string>;
    /**
     * 内容寻址布局（V2 §7.6）：scopedPath → blob 内容哈希（blobs/<hash> 寻址键）。
     * 提供时恢复文件从 blobsDir 读取；缺省（旧布局链节点）回退 backupDir 相对路径读取。
     * 内容寻址下 blob 不可变，fileHashes 的值与 blobHashes 恒等（同一 sha256）。
     */
    blobHashes?: Record<string, string>;
    /** 恢复后 best-effort 应用的权限位（scopedPath → mode；缺省不应用） */
...
export interface RestoreEngineOptions {
    /** 存档根目录（旧布局链节点的备份目录所在；越界防护基线） */
    checkpointsDir: string;
    /** 内容寻址 blob 池目录（`<dataRoot>/checkpoints/<workspace-id>/blobs`；缺省 = 旧布局） */
    blobsDir?: string;
    /** 当前工作区根目录集合 */
 ...
 * 键统一为 scoped 路径；备份源路径必须位于存档目录内
 * （backupDir 来自存档元数据，损坏数据可能含 `..`/绝对路径）。
 *
 * 内容寻址布局（V2 §7.6）：节点提供 blobHashes 时，文件从 blobsDir/<hash> 读取
 * （hash 经 sha256 hex 形状校验，防越界）；缺省回退 backupDir 相对路径（旧布局）。
 *
...
function buildFileIndex(
    ...
    const blobsRoot = blobsDir ? path.resolve(blobsDir) : undefined;
    ...
    for (const entry of chain) {
        ...
        for (const [rawKey, hash] of Object.entries(entry.fileHashes)) {
            ...
            // 内容寻址布局：blobsDir/<hash>（hash 即寻址键，同卷读取）
            if (blobsRoot && entry.blobHashes?.[rawKey]) {
                const blobHash = entry.blobHashes[rawKey]!;
                if (!/^[a-f0-9]{64}$/.test(blobHash)) continue;
                index.set(scopedKey, {
                    backupPath: path.resolve(blobsRoot, blobHash),
                    hash,
                    mode: entry.modes?.[rawKey]
                });
                continue;
            }

            // 旧布局：backupDir 相对路径（必须位于该节点目录内）
...
export async function restoreWorkspaceSnapshot(
    ...
    const fileIndex = buildFileIndex(chain, checkpointsDir, roots, options.blobsDir);
