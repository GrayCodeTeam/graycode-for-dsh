...
import { CheckpointIgnoreResolver, type CheckpointResolverExcludedEntry } from './CheckpointIgnoreResolver.ts';
import { isPathWithin } from './affectedPaths.ts';
import {
...
} from './CheckpointExclusionProfiles.ts';
import { runBounded } from './checkpointConcurrency.ts';
import { hashFileStreaming } from './fileHashing.ts';
import { isExcludedAbsolutePath } from './checkpointPathUtils.ts';
import type {
...
/** 单文件 stat 信息（bigint 提供纳秒精度 mtimeNs；mode 供内容寻址 manifest 记录权限位） */
