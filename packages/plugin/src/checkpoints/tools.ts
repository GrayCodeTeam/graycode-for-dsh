...
function createTool(service: CheckpointService) {
  return {
    ...
    checkpoint_restore: defineTool({
      ...
      description:
        'Restore a workspace to a checkpoint state. Requires the previewToken returned by checkpoint_preview in this session for the same checkpoint and workspace; the token binds a baseline digest of the workspace captured at preview time, so restoring is denied if any tracked file changed after preview (run checkpoint_preview again to re-confirm). Copies phase runs before any deletion and deletions only run when every copy succeeded; per-file failures are reported. With deleteUntrackedFiles=false (default) only files recorded by the snapshot are touched and files created after the snapshot are kept.',
      parameters: {
